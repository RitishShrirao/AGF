...
def get_loss_module(config):
    ...
    if task == "classification":
        ...
        if config["model"] in ["svd"]:
            return NoFussCE_AGF(reduction='none')
...
class NoFussCE_AGF(nn.CrossEntropyLoss):
    """
    pytorch's CrossEntropyLoss is fussy: 1) needs Long (int64) targets only, and 2) only 1D.
    This function satisfies these requirements
    """

    def forward(self, inp, target, eta, ortho_loss_list):
        loss_ce = F.cross_entropy(inp, target.long().squeeze(), weight=self.weight,
                               ignore_index=self.ignore_index, reduction=self.reduction)
        loss_ortho = sum(ortho_loss_list)/len(ortho_loss_list)

        loss_total = loss_ce + eta * loss_ortho
        return loss_total, loss_ce, loss_ortho
