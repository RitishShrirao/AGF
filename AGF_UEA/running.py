...
class SupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            padding_masks = padding_masks.to(self.device)  # 0s: ignore
            # regression: (batch_size, num_labels); classification: (batch_size, num_classes) of logits
            predictions, ortho_loss_list = self.model(X.to(self.device), padding_masks)

            loss, loss_ce, loss_ortho_loss = self.loss_module(predictions, targets, self.eta, ortho_loss_list)  # (batch_size,) loss for each sample in the batch
            batch_loss = torch.sum(loss)
            mean_loss = batch_loss / len(loss)  # mean loss (over samples) used for optimization
            ...
            batch_loss_ortho_loss = torch.sum(loss_ortho_loss)
            mean_loss_ortho_loss = batch_loss_ortho_loss / len(loss_ortho_loss)
            ...
            metrics = {"loss_total": mean_loss, "loss_ce": mean_loss_ce} #, "loss_ortho": mean_loss_ortho_loss}
            if i % self.print_interval == 0:
                ending = "" if epoch_num is None else 'Epoch {} '.format(epoch_num)
                self.print_callback(i, metrics, prefix='Training ' + ending)
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            loss, _, _ = self.loss_module(predictions, targets, self.eta, ortho_loss_list)
