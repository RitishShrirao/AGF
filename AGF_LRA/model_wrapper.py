...
class ModelForSC(nn.Module):
    ...
    def forward(self, input_ids_0, mask_0, label):
        ...
        with torch.cuda.amp.autocast(enabled = self.enable_amp):
            ...
            if self.attn_type.startswith("svd") or self.attn_type.startswith("evd"):
                token_out, ortho_loss_list = self.model(input_ids_0, mask_0)
            else:
            ...
            if self.attn_type.startswith("svd") or self.attn_type.startswith("evd"):
                ...
                ortho_loss = sum(ortho_loss_list) / len(ortho_loss_list)
                seq_loss = loss_ce + ortho_loss
            else:
...
class ModelForSCDual(nn.Module):
    ...
    def forward(self, input_ids_0, input_ids_1, mask_0, mask_1, label):
        ...
        with torch.cuda.amp.autocast(enabled = self.enable_amp):
            ...
            if  self.attn_type.startswith("svd") or self.attn_type.startswith("evd"):
                token_out_0, ortho_loss_list_0 = self.model(input_ids_0, mask_0)
                token_out_1, ortho_loss_list_1= self.model(input_ids_1, mask_1)
            else:
            ...
            if  self.attn_type.startswith("svd") or self.attn_type.startswith("evd"):
                loss_ce = torch.nn.CrossEntropyLoss(reduction = "none")(seq_scores, label)
                ortho_loss = 0
                for i in range(len(ortho_loss_list_0)):
                    ortho_loss += (ortho_loss_list_0[i] + ortho_loss_list_1[i])/2
                ortho_loss /= len(ortho_loss_list_0)
                seq_loss = loss_ce + ortho_loss
            else:
                seq_loss = torch.nn.CrossEntropyLoss(reduction = "none")(seq_scores, label)
