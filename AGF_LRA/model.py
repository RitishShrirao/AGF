...
class Transformer(nn.Module):
    ...
    def forward(self, X, mask, idx=0):
        if self.attn_type.startswith("svd") or self.attn_type.startswith("evd"):
            out, ortho_loss = self.mha(self.norm1(X), mask)
...
class Model(nn.Module):
    ...
    def forward(self, input_ids, mask = None):
        ...
        if self.attn_type.startswith("svd") or self.attn_type.startswith("evd"):
            ...
            if self.tied_weights:
                for idx in range(self.num_layers):
                    X, ortho_loss = self.transformer(X, mask)
                    ortho_loss_list.append(ortho_loss)
            else:
                for idx in range(self.num_layers):
                    X, ortho_loss = getattr(self, f"transformer_{idx}")(X, mask, idx)
                    ortho_loss_list.append(ortho_loss)
        ...
        if self.tied_weights:
            ...
            for idx in range(self.num_layers):
                X = getattr(self, f"transformer_{idx}")(X, mask, idx)
...
#                 X, ortho_loss = self.transformer1(X, mask)
#                 ortho_loss_list.append(ortho_loss)
#             else:

#                 X = getattr(self, f"transformer_0")(X, mask)
#                 X, ortho_loss = getattr(self, f"transformer_1")(X, mask)
#                 ortho_loss_list.append(ortho_loss)
