...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        if self.attn_type == "softmax":
            ...
            from attention_svd import SVDAttention
            self.attn = SVDAttention(config)
            self.W_Sigma = nn.Linear(self.dim, self.num_head * self.head_dim)
    ...
    def forward(self, X, mask):
        ...
        if self.attn_type.startswith("longformer") or self.attn_type.startswith("reformer"):
            ...
            Sigma = self.split_heads(self.W_Sigma(X))

            with torch.cuda.amp.autocast(enabled = False):
                if self.grad_checkpointing:
                    attn_out, ortho_loss = checkpoint(self.attn, Q.float(), Sigma.float(), K.float(), V.float(), mask.float())
                else:
                    attn_out, ortho_loss = self.attn(Q.float(), Sigma.float(), K.float(), V.float(), mask.float())
